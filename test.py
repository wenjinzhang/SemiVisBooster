...
def main_worker(gpu, ngpus_per_node, args):
    ...
    logger = get_logger(args.save_name, save_path, logger_level, eval=args.eval_only)
    ...
    args.load_path = args.load_path.replace('latest_model', 'model_best')
    ...
    import pickle
    # text_embs = model.proj_text()
    # print("saving projected text-embs!!!")
    # pickle.dump(text_embs.cpu(), open(f"{save_path}/text_embs.pk","wb"))

    print("evaluating!!!")
    eval_dict = model.evaluate(eval_dest='eval', return_logits=True)
    print(eval_dict['eval/top-1-acc'])
    pickle.dump(eval_dict, open(f"{save_path}/eval_dict2.pk","wb"))
