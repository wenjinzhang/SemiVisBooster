...
@ALGORITHMS.register('maepseudolabel')
class MAEPseudoLabel(AlgorithmBase):
    """
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w):
        ...
        with self.amp_cm():
            ...
            outs_x_lb = self.model(x_lb, need_mae=True)
            logits_x_lb = outs_x_lb['logits']
            feats_x_lb = outs_x_lb['feat']

            # calculate BN only for the first batch
            self.bn_controller.freeze_bn(self.model)
            outs_x_ulb = self.model(x_ulb_w, need_mae=True)
            ...
            mae_loss_x_lb = outs_x_lb['mae_loss'].sum() / outs_x_lb['mae_mask'].sum()
            mae_loss_x_ulb = outs_x_ulb['mae_loss'].sum() / outs_x_ulb['mae_mask'].sum()
            mae_loss = mae_loss_x_lb + mae_loss_x_ulb
            ...
            total_loss = sup_loss + self.lambda_u * unsup_loss * unsup_warmup + self.lambda_mae * mae_loss
