...
@ALGORITHMS.register('crosstextmatch')
class CrossTextMatch(AlgorithmBase):
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        ...
        self.vbank_ema_decay = args.vbank_ema_decay
        print(f"-----Apply Mixup----------->{self.apply_mixup}")

        # self.vision_embedding_bank = torch.zeros(self.num_classes, self.model.embed_dim).cuda()
        self.vision_embedding_bank = torch.zeros(self.num_classes, self.model.embed_dim, device='cuda', requires_grad=False)
    ...
    def update_vision_embedding_bank(self, vision_embeds, labels):
        """
        Update the vision embedding bank with EMA for each class.
        
        Args:
        - vision_embeds (torch.Tensor): Vision embeddings from the current batch.
        - labels (torch.Tensor): Class labels for each sample in the batch.
        """
        with torch.no_grad():
            for cls in labels.unique():
                class_mask = labels == cls
                class_mean = vision_embeds[class_mask].mean(dim=0).clone() 
                self.vision_embedding_bank[cls] = (
                    self.vbank_ema_decay * self.vision_embedding_bank[cls] + (1 - self.vbank_ema_decay) * class_mean
                )
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        projected_text_embeds_bank = self.model.module.projector(text_embs=self.text_embeddings_bank.detach(),
                                                                 vision_embs=self.vision_embedding_bank.detach())
        ...
        with self.amp_cm():
            ...
            self.update_vision_embedding_bank(vision_embeds.detach(), labels)
    ...
    def get_argument():
        return [
            ...
            SSL_Argument('--vbank_ema_decay', float, 0.99)
        ]