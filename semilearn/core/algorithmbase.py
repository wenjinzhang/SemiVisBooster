...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        # self.focal_loss = FocalLossV2(gamma=args.gamma)
    ...
    def set_dataset(self):
        ...
        if self.args.dataset in ["imagenet", "imagenet127"]:
            dataset_dict = get_dataset(self.args, self.algorithm, self.args.dataset, num_labels=self.args.num_labels, num_classes=self.args.num_classes, percentage=self.args.percentage, data_dir=self.args.data_dir, include_lb_to_ulb=self.args.include_lb_to_ulb)
        else:
            dataset_dict = get_dataset(self.args, self.algorithm, self.args.dataset, self.args.num_labels, self.args.num_classes, percentage= -1, data_dir=self.args.data_dir, include_lb_to_ulb=self.args.include_lb_to_ulb)
    ...
    def set_optimizer(self, additional_params=None):
        """
        ...
        if additional_params == None:
            ...
            self.print_fn("Recreate optimizer and scheduler")
        optimizer = get_optimizer(self.model, self.args.optim, self.args.lr, self.args.momentum, self.args.weight_decay, self.args.layer_decay, additional_params=additional_params)
        scheduler = get_cosine_schedule_with_warmup(optimizer,
                                                    self.num_train_iter,
                                                    num_warmup_steps=self.args.num_warmup_iter)
        return optimizer, scheduler

    def reset_schduler(self):
    ...
    def process_batch(self, input_args=None, **kwargs):
        ...
        for arg, var in kwargs.items():
            ...
            if isinstance(var, list):
                var = [x.cuda(self.gpu) for x in var ]
            else:
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                y_feats.append(feats.cpu().numpy())
                total_loss += loss.item() * num_batch

                if return_logits:
                    # Save images
                    if isinstance(x, dict):
                        x_images.append({k: v.cpu().numpy() for k, v in x.items()})
                    else:
                        x_images.append(x.cpu().numpy())
        ...
        # Generate indices for the rows and columns (0 to n-1, where n is the number of classes)
        # num_classes = cf_mat.shape[0]
        # indices = [f'Idx {i}' for i in range(num_classes)]

        # # Create a DataFrame for better readability with indices
        # cf_df = pd.DataFrame(cf_mat, index=indices, columns=indices)

        # # Print the confusion matrix with indices
        # self.print_fn('Confusion matrix with indices:\n' + cf_df.to_string())
        if hasattr(self, 'ema') and self.ema:
            self.ema.restore()
        self.model.train()
        ...
        if return_logits:
            ...
            eval_dict[eval_dest+'/feats'] = y_feats
            eval_dict[eval_dest+'/y_true'] = y_true
            eval_dict[eval_dest+'/y_pred'] = y_pred
            eval_dict[eval_dest+'/cf_mat'] = cf_mat
            eval_dict[eval_dest+'/x_images'] = x_images
