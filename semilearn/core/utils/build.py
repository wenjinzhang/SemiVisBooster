...
def get_logger(name, save_path=None, level='INFO', eval=False):
    """
    ...
    if not save_path is None:
        ...
        log_name = 'eval.txt' if eval else 'log.txt'
        fileHandler = logging.FileHandler(os.path.join(save_path, log_name))
...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, percentage=0.01, data_dir='./data', include_lb_to_ulb=True):
    """
    create dataset
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_food101, get_flowers102, get_json_dset, get_pkl_dset
    ...
    if dataset == "eurosat":
        ...
        lb_dset, ulb_dset, eval_dset = get_imagenet(args, algorithm, dataset, num_labels, percentage=percentage, data_dir=data_dir, include_lb_to_ulb=False)
        test_dset = None
    elif dataset in 'food101':
        lb_dset, ulb_dset, eval_dset = get_food101(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset in 'flowers102':
        lb_dset, ulb_dset, eval_dset = get_flowers102(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    # speech dataset
...
def get_optimizer(net, optim_name='SGD', lr=0.1, momentum=0.9, weight_decay=0, layer_decay=1.0, nesterov=True, bn_wd_skip=True, additional_params = None):
    '''
    ...
    if not additional_params is None:
        per_param_args.append(additional_params)
